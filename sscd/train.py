...
from sscd.models.model import Model, MAE
...
def add_train_args(parser: ArgumentParser, required=True):
    ...
    parser.add_argument("--optim", default="lars", choices=["lars", "adamw", "adam", "sgd"])
    ...
    parser.add_argument("--entropy_weight", default=1, type=float)
    ...
    parser.add_argument("--mixup", default=True, type=parse_bool)
    parser.add_argument(
    ...
    parser.add_argument(
        "--workers", default=16, type=int, help="Data loader workers per GPU process."
    )
...
def add_data_args(parser, required=True):
    ...
    parser.add_argument("--train_dataset_path", default="/data/DISC/images/dev_queries", type=str)
    parser.add_argument("--val_dataset_path", default="", required=False, type=str)
...
class DISCData(pl.LightningDataModule):
    ...
    def make_validation_dataset(
        ...
        return DISCEvalDataset(path, query_path=path, ref_path=path, transform=transforms, include_train=include_train)
...
class SSCD(pl.LightningModule):
    ...
    def __init__(self, args, train_steps: int):
        ...
        if args.mae:
            self.model = call_using_args(MAE, args)
            if args.ckpt:
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.loss(embeddings, instance_ids)
        return loss
